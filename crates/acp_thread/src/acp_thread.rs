...
use gpui::{
    ActivityGuard, App, AppContext, AsyncApp, Context, Entity, EventEmitter, SharedString, Subscription,
    Task, WeakEntity,
};
...
impl AssistantMessage {
    ...
    /// Used for syncing to external systems over the WebSocket.
...
pub struct ContextCompaction {
    ...
    pub error: Option<Entity<MessageText>>,
...
impl ContextCompaction {
        ...
        match update.error {
            ...
            MaybeUndefined::Value(error) => {
                if let Some(markdown) = &self.error {
                    markdown.update(cx, |markdown, _| markdown.reset(error));
                } else {
                    self.error = Some(cx.new(|_cx| MessageText::new(error)));
                }
            }
        }
    }
    ...
    fn append_summary(
        ...
        {
            markdown.update(cx, |markdown, _| markdown.append(&text.text));
        } else {
            self.summary
...
impl ToolCall {
    ...
    fn new_label(
        ...
        cx.new(|_cx| MessageText::new(text))
    }
        ...
        if label_changed {
            ...
            if was_plain_text != is_plain_text {
                ...
                if self.label.read(cx).source() != &text {
                    self.label.update(cx, |label, _| label.replace(text));
                }
            }
        }
...
impl MessageContent {
    ...
    pub fn markdowns(&self) -> impl Iterator<Item = &Entity<MessageText>> {
    ...
    fn trailing_text(&self) -> Option<&Entity<MessageText>> {
    ...
    pub fn new_output(
        ...
        match block {
            acp::ContentBlock::Text(text) => {
                Self::create_markdown_block(text.text, cx)
            }
            ...
            content => {
                ...
                Self::Unsupported {
                    content,
                    markdown: Self::create_markdown(description.into(), cx),
                }
            }
        }
    }
...
mod tests {
    ...
    async fn test_message_serialization_formats_are_pinned(cx: &mut gpui::TestAppContext) {
        ...
        cx.update(|cx| {
            ...
            assert_eq!(compaction.to_markdown(cx), "## Context Compaction (Completed)\n\n");
        });
    }
