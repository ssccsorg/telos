...
///
/// Sets up the external WebSocket sync and the agent thread handler so the
/// process can drive ACP threads from the WebSocket without any UI. The GPUI
/// event loop continues running until the process is signalled.
///
/// Without `external_websocket_sync` enabled this is effectively a no-op idle
/// loop, so the flag is mostly useful for the actus integration.
fn initialize_headless(app_state: Arc<AppState>, paths: Vec<String>, cx: &mut App) {
    log::info!("starting telos in headless mode (no windows, no display)");
    ...
    #[cfg(feature = "external_websocket_sync")]
    {
        ...
        // Without this, the project has zero worktrees and all file-based
        ...
        // Track the most recently displayed thread so the headless UI-state
        // responder can answer thread and entry-count queries faithfully.
        ...
        // Headless UI-state responder: synthesize a response from the headless
        // project's context_server_store, the most recently displayed thread,
        // and the global LanguageModelRegistry.
        ...
        cx.spawn(async move |cx| {
            while let Some(query) = query_rx.recv().await {
                ...
                if let Err(e) = send_result {
                    log::warn!("send_websocket_event(UiStateResponse) failed: {e:#}");
                }
            }
        })
        .detach();
        ...
        if settings.enabled && settings.websocket_sync.enabled {
            ...
            log::warn!(
                "external_websocket_sync feature is built in but settings disable it; \
    ...
    #[cfg(not(feature = "external_websocket_sync"))]
    {
        let _ = app_state;
        let _ = cx;
        log::warn!("built without external_websocket_sync; --headless has nothing to do");
    }
}
...
struct Args {
    ...
    /// Disable single instance check, allowing multiple instances.
    /// Required by the external_websocket_sync integration.
    #[arg(long)]
    allow_multiple_instances: bool,

    /// Run without a display server and without opening windows. Initializes
    /// the external WebSocket sync and the agent backend only.
    #[arg(long)]
    headless: bool,
